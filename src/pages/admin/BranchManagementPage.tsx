...
/** Store entity: store_id, store_name, address, phone, manager_id (FK User) */
interface Store {
  store_id: number;
  store_name: string;
  address: string;
...
/** Mock users for manager dropdown (chỉ cần id + full_name) */
const MOCK_USERS = [
...
const MOCK_STORES: Store[] = [
  { store_id: 1, store_name: 'Cửa hàng Quận 1', address: '123 Nguyễn Huệ, Q1, TP.HCM', phone: '028 1234 5678', manager_id: 1 },
  { store_id: 2, store_name: 'Cửa hàng Quận 3', address: '45 Lê Lợi, Q3, TP.HCM', phone: '028 2345 6789', manager_id: 2 },
  { store_id: 3, store_name: 'Cửa hàng Quận 7', address: '78 Nguyễn Văn Linh, Q7, TP.HCM', phone: '028 3456 7890', manager_id: null },
];
...
const BranchManagementPage = () => {
  ...
  const [formData, setFormData] = useState<{
    ...
    phone: string;
    manager_id: number | null;
  }>({ store_name: '', address: '', phone: '', manager_id: null });
    ...
    <>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filteredStores.map((store) => (
                    <tr key={store.store_id} className="border-b border-amber-100/80 transition-colors hover:bg-amber-50/70">
                      ...
                      <td className="px-5 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          <Button variant="outline" size="icon" className="size-8 border-amber-200 text-amber-700 hover:bg-amber-50 hover:text-amber-800" onClick={() => openEdit(store)}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent onClose={() => setDialogOpen(false)} className="max-w-2xl min-w-[28rem] p-8">
          <DialogHeader>
            <DialogTitle className="text-xl">{editingStore ? 'Chỉnh sửa cửa hàng' : 'Thêm cửa hàng mới'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-5 py-2">
            <div className="space-y-2">
              <Label htmlFor="store_name" className="text-base">Tên cửa hàng</Label>
              <Input
                id="store_name"
            ...
            <div className="space-y-2">
              <Label htmlFor="manager_id" className="text-base">Cửa hàng trưởng (User)</Label>
              <select
                id="manager_id"
